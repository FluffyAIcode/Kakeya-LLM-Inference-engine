...
| **KIE-v1.1.z** | **throughput + N=75**: (z.1) graph capture / `torch.compile` decode, (z.2) fused quantized-attention kernel, (z.3) continuous-batched prefill, (z.4) fuse kakeyalattice v1.6.1 packed exact storage (2.46× → N=75). Target: N=75 + decode tok/s ≥ vLLM. | in progress | #139 |
